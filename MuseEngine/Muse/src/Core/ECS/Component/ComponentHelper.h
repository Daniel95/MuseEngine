...
namespace Muse
{
	...
	std::unordered_map<int, ComponentPair<T1, T2>> ComponentHelper::GetComponentPairCollection()
	{
		...
		const std::unordered_map<int, T1>& components1 = Muse::ComponentManager<T1>::GetComponents();
		const std::unordered_map<int, T2>& components2 = Muse::ComponentManager<T2>::GetComponents();
