...
namespace Muse
{
	...
	const glm::vec3 & RefractiveMaterial::GetColor(std::shared_ptr<const RenderComponent> a_RenderComponent, const glm::vec3& point, std::shared_ptr<GetColorParameters> getColorParameters) const
	{
	...
	const glm::vec3 & RefractiveMaterial::GetRefraction(std::shared_ptr<const RenderComponent> a_RenderComponent, const glm::vec3& point, std::shared_ptr<GetColorParameters> getColorParameters) const
	{
		...
		if (a_RenderComponent->GetGameObject()->GetScene()->RayCast(rayHitDatas, renderComponents, ray))
		{
			...
			refractionColor = closestHit->m_RenderComponent->GetColor(closestHit->m_IntersectionPoint, getColorParameters);
		}
